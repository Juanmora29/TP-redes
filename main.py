...
# =========================================================================
# <<< ¡CORRECCIÓN AQUÍ! >>>
# Declaramos las constantes globales al principio, después de los imports.
# Así, estarán disponibles para todas las funciones que vengan después.
# =========================================================================
...
# --- GESTOR DE LIFESPAN Y DATOS EN MEMORIA ---
movies_db: List[Dict] = []
...
async def limitador_de_tasa(request: Request, call_next):
    ...
    if len(historial_ip) >= MAX_PETICIONES:
        raise HTTPException(
            ...
            detail=f"Límite de solicitudes alcanzado ({MAX_PETICIONES} por segundo). Inténtalo de nuevo más tarde.",
        )
...
# Ahora esta función no dará error porque DATA_FILE y REMOTE_URL ya existen.
def initialize_data():
    global movies_db
...
# ... (El resto del código: modelos, seguridad y endpoints no necesita cambios) ...
